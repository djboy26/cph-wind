...
export interface WindArrowInstance {
  ...
  phase0: number;
  /** Animation rate multiplier (stronger wind drifts faster). */
  speedFactor: number;
  /** Rendered arrow length, meters (≈ 1/5 of carriageway width). */
  arrowSizeM: number;
  /** Local wind speed, m/s (drives color). */
  speedMs: number;
...
function cycleOf(d: WindArrowInstance, flowPhase: number): number {
  return ((flowPhase * BASE_RATE * d.speedFactor + d.phase0) % 1 + 1) % 1;
}
...
function arrowColor(d: WindArrowInstance, flowPhase: number): [number, number, number, number] {
  ...
  const alpha = Math.round(Math.sin(Math.PI * cycle) * 235) + 20;
  return [d.color[0], d.color[1], d.color[2], Math.min(255, alpha)];
}
